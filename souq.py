...
# open csv file
csv_file = open("products.csv", "w", encoding="utf-8-sig")  # encoding="utf-8-sig" ==> to allow non English letters (Arabic)
field_names = ["Title", "Price", "Feauters", "Sim Card", "More Info", "Product Image"]
...
for page_num in range(1000):
    try:
        ...
        sub_titles_list = [title.text.strip() for title in soup.find_all('h1', {'class': 'itemTitle'})]
        sub_prices_list = [price.text.strip() for price in soup.find_all('h3', {'class': 'itemPrice'})]
        sub_imgs_list = [div.find('img')['data-src'] for div in soup.find_all('div', {'class': 'column column-block block-list-large single-item'})]
        sub_feauters = [feature.text.strip().replace("\n", " || ") for feature in soup.find_all('ul', {'class': 'selling-points'})]
        sub_sim_cards = [card.text.strip() for card in soup.find_all('a', {'class': 'sk-clr2 changeVariance is-active'})]
        sub_links = [link['href'] for link in soup.find_all('a', {'class': 'view-product-details sPrimaryLink secondary button expand white tiny'})]
...
json.dump(data, json_file, ensure_ascii=False)  # In JSON files [ensure_ascii=False] ==> to allow non English letters (Arabic)
